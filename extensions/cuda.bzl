...
def _cuda_impl(mctx):
    ...
    for tag in _version_tags(mctx):
        ...
        repo_name = _version_repo_name(tag.cuda_version)
        if repo_name in seen_repo_names:
            prev = seen_repo_names[repo_name]
            if prev["cuda_umd_version"] != cuda_umd_version or prev["cudnn_version"] != tag.cudnn_version:
                fail(
                    ("Conflicting configuration for cuda_version '{cuda_version}'. " +
                     "Expected cudnn_version={prev_cudnn}, cuda_umd_version={prev_umd}; got " +
                     "cudnn_version={new_cudnn}, cuda_umd_version={new_umd}.").format(
                        cuda_version = tag.cuda_version,
                        prev_cudnn = prev["cudnn_version"],
                        prev_umd = prev["cuda_umd_version"],
                        new_cudnn = tag.cudnn_version,
                        new_umd = cuda_umd_version,
                    ),
                )
            continue
        seen_repo_names[repo_name] = {
            "cudnn_version": tag.cudnn_version,
            "cuda_umd_version": cuda_umd_version,
        }
